...
/*	
	...
	*/
	//icomoon
	$conexi_matches = get_conexi_icons();
	foreach($conexi_matches as $match)
	{
		$icons[] = array('value' => 'icon '.$match[1], 'label' => $match[1]);
	}
...
*/
function get_conexi_icons()
{
	$pattern = '/\.(conexi-(?:\w+(?:-)?)+):before\s*{\s*content/';
	$subject = file_get_contents(get_template_directory_uri() . '/assets/fonts/conexi-icon/style.css');
	preg_match_all($pattern, $subject, $matches, PREG_SET_ORDER);
	return $matches;
}
//get blog categories
...
function conexi_the_pagination2($args = array(), $echo = 1)
{
...
function conexi_trim( $text, $len, $more = null )
	{
...
add_filter('conexi_redux_custom_fonts_load', 'conexi_redux_custom_fonts_load');
function conexi_redux_custom_fonts_load( $custom_font ) {
	$custom_style = '';
	...
	if ( $filename = conexi_set( $pathinfo, 'filename' ) ) {
		...
		foreach ( $extensions as $extension ) {
			$file_path = esc_url(home_url('/')) . '/wp-content/themes/conexi/assets/css/custom-fonts/' . $filename . '.' . $extension;
...
/**
 * [conexi_social_share_output description]
 *
 ...
 */
function conexi_social_share_output( $icon, $color = false ) {
...
add_action('conexi_social_share_output', 'conexi_social_share_output');
