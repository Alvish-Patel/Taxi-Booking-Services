...
class Common {
	...
	function blog( $template = 'blog' ) {
		...
		if ( ( $wp_query->is_posts_page && 'blog' == $template ) || $template == 'page' ) {
			...
			$return = [
				'layout'            => conexi_meta( 'sidebar_sidebar_layout', $page_id, 'right' ),
				'sidebar_type'      => conexi_meta( 'sidebar_source_type', $page_id, 'd' ),
				'sidebar_elementor' => conexi_meta( 'sidebar_elementor_template', $page_id ),
				'sidebar'           => conexi_meta( 'sidebar_page_sidebar', $page_id, 'default-sidebar' ),
				'banner_type'       => conexi_meta( 'banner_source_type', $page_id ),
				'banner_elementor'  => conexi_meta( 'banner_elementor_template', $page_id ),
				'banner'            => conexi_set( conexi_meta( 'banner_page_background', $page_id ), 'url' ),
				'title'             => conexi_meta( 'banner_banner_title', $page_id ),
				'enable_banner'     => conexi_meta( 'banner_page_banner', $page_id ),
			];
			...
			$return = [
				...
				'title'         => $options->get( $title ) ? $options->get( $title ) : conexi_the_title( $template ),
				'banner'        => conexi_set( $bg, 'url' ),
	...
	public function single( $template = 'single' ) {
		...
		$return = [
			...
			'banner_elementor'  => conexi_meta( 'banner_elementor_template', $page_id ),
			'banner'            => conexi_set( conexi_meta( 'banner_page_background', $page_id ), 'url' ),
			'title'             => ($title) ? $title : esc_html__('Standard Post', 'conexi'),
			'enable_banner'     => conexi_meta( 'banner_page_banner', $page_id ),
