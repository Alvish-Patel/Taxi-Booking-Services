...
class Options {
	 ...
	 * @var string
	 */
	private $opt_name = CONEXI_NAME . '_options';
	...
	private $page_slug = CONEXI_NAME . '_options';
	...
	function init() {
		...
		$this->menu_title = esc_html__( 'CONEXI Options', 'conexi' );
		$this->page_title = esc_html__( 'CONEXI Options', 'conexi' );
	...
	function args() {
		...
		$args = array(
			...
			'global_variable'      => 'conexi_options',
	...
	function helpTabs() {
		...
		$tabs = array(
			array(
				'id'      => 'redux-help-tab-1',
				'title'   => esc_html__( 'Theme Information 1', 'conexi' ),
				'content' => esc_html__( '<p>This is the tab content, HTML is allowed.</p>', 'conexi' ),
			),
			array(
				'id'      => 'redux-help-tab-2',
				'title'   => esc_html__( 'Theme Information 2', 'conexi' ),
				'content' => esc_html__( '<p>This is the tab content, HTML is allowed.</p>', 'conexi' ),
			),
		);
		...
		$content = '<p>' . esc_html__( 'This is the sidebar content, HTML is allowed.', 'conexi' ) . '</p>';
	...
	function sections() {
		...
		$sections_path = apply_filters( 'conexi_redux_sections', $sections_path );
		...
		foreach ( $sections_path as $key => $file ) {
			...
			if ( file_exists( $file ) ) {
				...
				$options = apply_filters( "conexi_redux_sections_{$key}", $options );
